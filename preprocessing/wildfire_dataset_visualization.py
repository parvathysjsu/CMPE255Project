...
def graphs_from_preprocessed_file(df):
    ...
    fig, ax = plt.subplots(figsize=(22,15))
    df1 = df
    df1.dropna(subset = ["COUNTY"], inplace=True)
    county_list1= df1['COUNTY'].tolist()
    #print(len(county_list1))
    unique, counts = np.unique(county_list1, return_counts=True)
    plt.plot(unique, counts, 1)
    plt.title('County-wise Fires')
    plt.xlabel('COUNTY', fontsize=9)
    plt.ylabel('FIRES')
    plt.xticks(rotation=90)
